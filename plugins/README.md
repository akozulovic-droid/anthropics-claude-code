...
| [vercel](./vercel/) | Deploy and manage Vercel projects — deployments, environment variables, logs, and domain management | **MCP Server:** `vercel` - Official Vercel MCP server<br>**Commands:** `/vercel:deploy`, `/vercel:env`, `/vercel:logs` - Deploy, manage env vars, view logs<br>**Agent:** `vercel-deploy-checker` - Diagnose deployment failures<br>**Skill:** `vercel-deployment` - Auto-invoked expertise on Vercel config and features |
